...
第一阶段已经从纯确定性规则升级为“两阶段 LLM/RAG 推理 + 规则兜底”。无 `OPENAI_API_KEY` 时，工具会用本地知识库检索和规则链路兜底；配置 Key 后，先由 LLM 基于原始需求 intake 拆分可测试需求单元并列出待澄清项，用户补充澄清后，再由 LLM/RAG 推理技术影响、测试点、测试用例和 Harness Gate 结果。
...
- Agent 资产视图，展示 Product / Dev / Test / Review Agent 的结构化输出。
- 技术影响分析。
- 测试点提取。
- 功能测试策略生成。
- 功能测试用例生成。
...
- 保存每个中间产物。
- 支持人工评审 P0/P1 用例。
- 为 LLM 需求拆分和用例生成结果建立离线评测集和采纳率统计。
